...
export async function generateMetadata({ params }: Props) {
  const article = getArticleByRoute(`/assam-administration/${params.slug}/`);
  if (!article) return {};
...
export default function AssamAdministrationArticlePage({ params }: Props) {
  const article = getArticleByRoute(`/assam-administration/${params.slug}/`);
  ...
  const related = getArticlesByCategory('assam-administration')
      ...
      <header className="glass-panel p-6 sm:p-10 rounded-3xl space-y-6 border-[#E5DAC8]">
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs font-mono font-bold text-[#9E3A26]">
          <span className="bg-[#9E3A26]/10 px-3 py-1 rounded-md border border-[#9E3A26]/20 uppercase tracking-wider">
            Assam Administration & Governance
          </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <main className="lg:col-span-3 bg-[#F3EDE2] p-6 sm:p-10 rounded-3xl border border-[#E5DAC8] shadow-sm space-y-6">
          ...
          <div className="mt-10 p-6 rounded-2xl glass-panel border-[#E5DAC8] text-center space-y-3">
            <h3 className="text-lg font-bold text-[#1D1814]">Practice Official Assam Polity & Administration Questions</h3>
            <p className="text-xs text-[#6B5D52]">Solve solved question sets for Assam governance, schemes, and administration.</p>
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <Link href="/adre/syllabus/polity/" className="btn-ahom-primary text-xs inline-block">
                View Polity Syllabus →
              </Link>

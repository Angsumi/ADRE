...
export async function generateMetadata({ params }: Props) {
  const article = getArticleByRoute(`/adre-eligibility/${params.slug}/`);
  if (!article) return {};
...
export default function AdreEligibilityArticlePage({ params }: Props) {
  const article = getArticleByRoute(`/adre-eligibility/${params.slug}/`);
  ...
  const related = getArticlesByCategory('adre-eligibility')
      ...
      <header className="glass-panel p-6 sm:p-10 rounded-3xl space-y-6 border-[#E5DAC8]">
        ...
        <div className="pt-2">
          <Link
            ...
            className="btn-ahom-primary text-xs inline-block font-bold"
          >
            ⚡ Test Your Criteria in Interactive Eligibility Checker →
          </Link>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <main className="lg:col-span-3 bg-[#F3EDE2] p-6 sm:p-10 rounded-3xl border border-[#E5DAC8] shadow-sm space-y-6">
          ...
          <div className="mt-10 p-6 rounded-2xl glass-panel border-[#E5DAC8] text-center space-y-3">
            <h3 className="text-lg font-bold text-[#1D1814]">Want to check your exact age & post eligibility instantly?</h3>
            <p className="text-xs text-[#6B5D52]">Use our free tool to verify your Grade 3 & Grade 4 post qualifications in seconds.</p>
            <Link href="/tools/eligibility-checker/" className="btn-ahom-primary text-xs inline-block">

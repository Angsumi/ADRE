...
export async function generateStaticParams() {
  const articles = getArticlesByCategory('adre-comparisons');
...
export async function generateMetadata({ params }: Props) {
  const article = getArticleByRoute(`/adre-comparisons/${params.slug}/`);
  if (!article) return {};
...
export default function AdreComparisonArticlePage({ params }: Props) {
  const article = getArticleByRoute(`/adre-comparisons/${params.slug}/`);
  ...
  const breadcrumbs = generateBreadcrumbSchema([
    ...
    { name: 'Comparisons', url: '/adre-comparisons/' },
    { name: article.h1, url: article.route },
  ]);

  const related = getArticlesByCategory('adre-comparisons')
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 bg-[#F9F4EB]">
      ...
      <header className="glass-panel p-6 sm:p-10 rounded-3xl space-y-6 border-[#E5DAC8]">
        ...
        <p className="text-sm sm:text-base text-[#6B5D52] font-medium leading-relaxed max-w-4xl border-l-4 border-[#C9A468] pl-4 bg-[#F3EDE2]/60 py-2 rounded-r-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <main className="lg:col-span-3 bg-[#F3EDE2] p-6 sm:p-10 rounded-3xl border border-[#E5DAC8] shadow-sm space-y-6">
          ...
          <div className="mt-10 p-6 rounded-2xl glass-panel border-[#E5DAC8] text-center space-y-3">
            <h3 className="text-lg font-bold text-[#1D1814]">Looking for Grade-Specific Preparation Materials?</h3>
            <p className="text-xs text-[#6B5D52]">Explore dedicated syllabus blueprints, previous year papers, and practice sets.</p>
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <Link href="/adre/grade-3/" className="btn-ahom-primary text-xs inline-block">
                Grade 3 Prep Hub →
              </Link>
              <Link href="/adre/grade-4/" className="px-4 py-2.5 rounded-xl border border-[#9E3A26] text-[#9E3A26] font-bold text-xs hover:bg-[#9E3A26]/10 transition-colors">
                Grade 4 Prep Hub →
              </Link>
            </div>
          </div>

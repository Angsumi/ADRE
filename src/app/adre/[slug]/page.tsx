...
export async function generateStaticParams() {
  const articles = getArticlesByCategory('adre');
...
export async function generateMetadata({ params }: Props) {
  const article = getArticleByRoute(`/adre/${params.slug}/`);
  if (!article) return {};
...
export default function AdreArticlePage({ params }: Props) {
  const article = getArticleByRoute(`/adre/${params.slug}/`);
  ...
  const relatedArticles = getArticlesByCategory('adre')
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 bg-[#F9F4EB]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <main className="lg:col-span-3 bg-[#F3EDE2] p-6 sm:p-10 rounded-3xl border border-[#E5DAC8] shadow-sm space-y-6">
          ...
          {/* Practice & Mock Test CTAs */}
          <div className="mt-10 p-6 rounded-2xl glass-panel border-[#E5DAC8] text-center space-y-3">
            <h3 className="text-lg font-bold text-[#1D1814]">Ready to put your preparation to the test?</h3>
            <p className="text-xs text-[#6B5D52]">Solve official ADRE solved question papers and full-length timed mock tests.</p>
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <Link href="/adre/mock-test/" className="btn-ahom-primary text-xs inline-block">
                Start Free Full Mock Test →
              </Link>
              <Link href="/adre/syllabus/" className="px-4 py-2.5 rounded-xl border border-[#9E3A26] text-[#9E3A26] font-bold text-xs hover:bg-[#9E3A26]/10 transition-colors">
                View Interactive Syllabus →
              </Link>
            </div>
          </div>
        </main>

...
export async function generateMetadata({ params }: Props) {
  const article = getArticleByRoute(`/assam-culture/${params.slug}/`);
  if (!article) return {};
...
export default function AssamCultureArticlePage({ params }: Props) {
  const article = getArticleByRoute(`/assam-culture/${params.slug}/`);
  ...
  const related = getArticlesByCategory('assam-culture')
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <main className="lg:col-span-3 bg-[#F3EDE2] p-6 sm:p-10 rounded-3xl border border-[#E5DAC8] shadow-sm space-y-6">
          ...
          <div className="mt-10 p-6 rounded-2xl glass-panel border-[#E5DAC8] text-center space-y-3">
            <h3 className="text-lg font-bold text-[#1D1814]">Practice Official Assam Culture & History MCQs</h3>
            <p className="text-xs text-[#6B5D52]">Solve solved question sets for Assam GK, history, and culture topics.</p>
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <Link href="/assam-gk/" className="btn-ahom-primary text-xs inline-block">
                Practice Assam GK MCQs →
              </Link>
